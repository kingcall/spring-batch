...
public class FlatFileBatchItemWriterJob {
    @Autowired
    ...
    @Bean
    public Job flatFileBatchItemWriterJoB() {
        return jobBuilderFactory
                .get(this.getClass().getSimpleName())
                .start(flatFileBatchItemWriterJobStep())
                .build()
                ;
    }

    @Bean
    public Step flatFileBatchItemWriterJobStep() {
        return stepBuilders
    ...
    @Bean
    public FlatFileItemWriter<Person> flatFileWriter() {
        FlatFileItemWriter<Person> writer = new FlatFileItemWriter<>();
        writer.setResource(new FileSystemResource("data/csv/person_write.csv"));
        //writer.setResource(new ClassPathResource("csv/person_write.csv"));
        // 如何将对象转化为一行
